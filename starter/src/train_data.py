...
from prepare_data import process_data, remove_spaces
import model
...
def split_and_train():
    ...
    os.makedirs("starter/data/prepared", exist_ok=True)
    # save processed train/test data
    with open("starter/data/prepared/train.pkl", 'wb') as f:
            pickle.dump(Xy_train, f)

    with open("starter/data/prepared/test.pkl", 'wb') as f:
    ...
    # save model to disk
    with open('starter/model/rfc.pkl', 'wb') as file:
